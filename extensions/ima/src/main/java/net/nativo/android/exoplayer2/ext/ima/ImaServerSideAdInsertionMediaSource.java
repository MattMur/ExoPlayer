...
import net.nativo.android.exoplayer2.source.CompositeMediaSource;
...
import net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionMediaSource.AdPlaybackStateUpdater;
...
public final class ImaServerSideAdInsertionMediaSource extends CompositeMediaSource<Void> {
  ...
  private void invalidateServerSideAdInsertionAdPlaybackState() {
    if (!adPlaybackState.equals(AdPlaybackState.NONE) && contentTimeline != null) {
      ImmutableMap<Object, AdPlaybackState> splitAdPlaybackStates =
          ImaUtil.splitAdPlaybackStateForPeriods(adPlaybackState, contentTimeline);
  ...
  private static AdPlaybackState setVodAdGroupPlaceholders(
    ...
    for (int i = 0; i < cuePoints.size(); i++) {
      ...
      long fromPositionUs = msToUs(ImaUtil.secToMsRounded(cuePoint.getStartTime()));
      adPlaybackState =
  ...
  private static long getAdDuration(double startTimeSeconds, double endTimeSeconds) {
    ...
    return msToUs(ImaUtil.secToMsRounded(endTimeSeconds - startTimeSeconds));
  }
    ...
    if (adGroup.count < adPodInfo.getTotalAds()) {
      adPlaybackState =
          ImaUtil.expandAdGroupPlaceholder(
              adGroupIndex,
              /* adGroupDurationUs= */ msToUs(ImaUtil.secToMsRounded(adPodInfo.getMaxDuration())),
              adIndexInAdGroup,
              /* adDurationUs= */ msToUs(ImaUtil.secToMsRounded(ad.getDuration())),
          ...
          if (isLiveStream) {
            ...
            newAdPlaybackState =
                ImaUtil.addLiveAdBreak(
                    ...
                    /* adDurationUs= */ ImaUtil.secToUsRounded(ad.getDuration()),
                    /* adPositionInAdPod= */ adPodInfo.getAdPosition(),
                    /* totalAdDurationUs= */ ImaUtil.secToUsRounded(adPodInfo.getMaxDuration()),
  ...
  private static class StreamManagerLoadable
    ...
    private final ImaUtil.ServerSideAdInsertionConfiguration serverSideAdInsertionConfiguration;
    ...
    private StreamManagerLoadable(
        com.google.ads.interactivemedia.v3.api.AdsLoader adsLoader,
        ImaUtil.ServerSideAdInsertionConfiguration serverSideAdInsertionConfiguration,
