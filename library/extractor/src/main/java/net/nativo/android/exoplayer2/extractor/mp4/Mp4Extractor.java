...
import net.nativo.android.exoplayer2.extractor.TrueHdSampleRechunker;
import net.nativo.android.exoplayer2.metadata.Metadata;
import net.nativo.android.exoplayer2.metadata.mp4.MotionPhotoMetadata;
...
public final class Mp4Extractor implements Extractor, SeekMap {
  ...
  private boolean readAtomHeader(ExtractorInput input) throws IOException {
    ...
    if (atomSize == Atom.DEFINES_LARGE_SIZE) {
      ...
      if (endPosition == C.LENGTH_UNSET) {
        @Nullable Atom.ContainerAtom containerAtom = containerAtoms.peek();
  ...
  private static @FileType int brandToFileType(int brand) {
    switch (brand) {
      case Sniffer.BRAND_QUICKTIME:
        return FILE_TYPE_QUICKTIME;
      case Sniffer.BRAND_HEIC:
