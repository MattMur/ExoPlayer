...
import net.nativo.android.exoplayer2.source.hls.HlsDataSourceFactory;
...
public final class DefaultHlsPlaylistTracker
  ...
  private boolean maybeSelectNewPrimaryUrl() {
    ...
    for (int i = 0; i < variantsSize; i++) {
      MediaPlaylistBundle bundle = Assertions.checkNotNull(playlistBundles.get(variants.get(i).url));
  ...
  private boolean notifyPlaylistError(
          Uri playlistUrl, LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo, boolean forceRetry) {
  ...
  private class FirstPrimaryMediaPlaylistListener implements PlaylistEventListener {
    ...
    public boolean onPlaylistError(Uri url, LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo, boolean forceRetry) {
      if (primaryMediaPlaylistSnapshot == null) {
        ...
        List<HlsMultivariantPlaylist.Variant> variants = Util.castNonNull(multivariantPlaylist).variants;
        for (int i = 0; i < variants.size(); i++) {
          @Nullable
