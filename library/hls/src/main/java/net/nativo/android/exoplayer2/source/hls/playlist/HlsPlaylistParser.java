 ...
 */
package net.nativo.android.exoplayer2.source.hls.playlist;
...
import net.nativo.android.exoplayer2.extractor.mp4.PsshAtomUtil;
import net.nativo.android.exoplayer2.metadata.Metadata;
import net.nativo.android.exoplayer2.source.hls.HlsTrackMetadataEntry;
import net.nativo.android.exoplayer2.source.hls.HlsTrackMetadataEntry.VariantInfo;
...
import net.nativo.android.exoplayer2.source.hls.playlist.HlsMultivariantPlaylist.Rendition;
...
public final class HlsPlaylistParser implements ParsingLoadable.Parser<HlsPlaylist> {
  ...
  private static HlsMediaPlaylist parseMediaPlaylist(
    ...
    while (iterator.hasNext()) {
      ...
      if (line.startsWith(TAG_PLAYLIST_TYPE)) {
        ...
        int startIndex = (int) (mediaSequence - Util.castNonNull(previousMediaPlaylist).mediaSequence);
  ...
  private static DrmInitData getPlaylistProtectionSchemes(
      @Nullable String encryptionScheme, DrmInitData.SchemeData[] schemeDatas) {
  ...
  private static long parseOptionalLongAttr(String line, Pattern pattern, long defaultValue) {
    Matcher matcher = pattern.matcher(line);
    if (matcher.find()) {
      return Long.parseLong(Assertions.checkNotNull(matcher.group(1)));
    }
    return defaultValue;
  }
  ...
  private static @PolyNull String parseOptionalStringAttr(
    ...
    Matcher matcher = pattern.matcher(line);
    @PolyNull String value = matcher.find() ? Assertions.checkNotNull(matcher.group(1)) : defaultValue;
  ...
  private static double parseOptionalDoubleAttr(String line, Pattern pattern, double defaultValue) {
    Matcher matcher = pattern.matcher(line);
    if (matcher.find()) {
      return Double.parseDouble(Assertions.checkNotNull(matcher.group(1)));
    }
    return defaultValue;
  }
