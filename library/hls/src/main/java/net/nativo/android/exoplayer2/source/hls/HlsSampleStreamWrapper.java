...
/* package */ final class HlsSampleStreamWrapper
  ...
  public void onPlaylistUpdated() {
    ...
    if (chunkState == HlsChunkSource.CHUNK_PUBLICATION_STATE_PUBLISHED) {
      lastMediaChunk.publish();
    } else if (chunkState == HlsChunkSource.CHUNK_PUBLICATION_STATE_REMOVED
        && !loadingFinished
        && loader.isLoading()) {
      loader.cancelLoading();
    }
    ...
    if (!forceRetry) {
      ...
      LoadErrorHandlingPolicy.FallbackSelection fallbackSelection =
          loadErrorHandlingPolicy.getFallbackSelectionFor(
              TrackSelectionUtil.createFallbackOptions(chunkSource.getTrackSelection()), loadErrorInfo);
  ...
  public void reevaluateBuffer(long positionUs) {
    ...
    while (newQueueSize > 0
        && chunkSource.getChunkPublicationState(readOnlyMediaChunks.get(newQueueSize - 1))
            == HlsChunkSource.CHUNK_PUBLICATION_STATE_REMOVED) {
      newQueueSize--;
    }
