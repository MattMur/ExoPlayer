...
class AsynchronousMediaCodecBufferEnqueuer {
  ...
  public void queueInputBuffer(
    ...
    Message message = Util.castNonNull(handler).obtainMessage(MSG_QUEUE_INPUT_BUFFER, messageParams);
  ...
  private void flushHandlerThread() throws InterruptedException {
    Assertions.checkNotNull(this.handler).removeCallbacksAndMessages(null);
    blockUntilHandlerThreadIsIdle();
  }

  private void blockUntilHandlerThreadIsIdle() throws InterruptedException {
    conditionVariable.close();
    Assertions.checkNotNull(handler).obtainMessage(MSG_OPEN_CV).sendToTarget();
  ...
  private static void copy(
    ...
    frameworkCryptoInfo.key = Assertions.checkNotNull(copy(cryptoInfo.key, frameworkCryptoInfo.key));
    frameworkCryptoInfo.iv = Assertions.checkNotNull(copy(cryptoInfo.iv, frameworkCryptoInfo.iv));
