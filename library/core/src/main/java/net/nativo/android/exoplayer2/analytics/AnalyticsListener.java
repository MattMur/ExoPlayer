...
import net.nativo.android.exoplayer2.trackselection.TrackSelection;
...
public interface AnalyticsListener {
  ...
  final class Events {
    ...
    public Events(FlagSet flags, SparseArray<EventTime> eventTimes) {
      ...
      for (int i = 0; i < flags.size(); i++) {
        ...
        flagsToTimes.append(eventFlag, Assertions.checkNotNull(eventTimes.get(eventFlag)));
      }
    ...
    public EventTime getEventTime(@EventFlags int event) {
      return Assertions.checkNotNull(eventTimes.get(event));
    }
   ...
   *
   * @param eventTime The event time.
   * @param playbackSuppressionReason The new {@link Player.PlaybackSuppressionReason}.
   */
   ...
   */
  @Deprecated
  default void onPositionDiscontinuity(EventTime eventTime, @Player.DiscontinuityReason int reason) {}
