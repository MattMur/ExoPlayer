...
import net.nativo.android.exoplayer2.util.Consumer;
...
/* package */ class DefaultDrmSession implements DrmSession {
  ...
  @Nullable public final List<DrmInitData.SchemeData> schemeDatas;
  ...
  @Nullable private ExoMediaDrm.KeyRequest currentKeyRequest;
  @Nullable private ExoMediaDrm.ProvisionRequest currentProvisionRequest;
  ...
  public boolean requiresSecureDecoder(String mimeType) {
    return mediaDrm.requiresSecureDecoder(Assertions.checkStateNotNull(sessionId), mimeType);
  }
    ...
    public void handleMessage(Message msg) {
      ...
      try {
        switch (msg.what) {
          case MSG_PROVISION:
            response =
                callback.executeProvisionRequest(uuid, (ExoMediaDrm.ProvisionRequest) requestTask.request);
            break;
          case MSG_KEYS:
            response = callback.executeKeyRequest(uuid, (ExoMediaDrm.KeyRequest) requestTask.request);
            break;
          default:
