...
/* package */ final class MediaPeriodQueue {
   ...
   */
  public boolean updateRepeatMode(Timeline timeline, @Player.RepeatMode int repeatMode) {
  ...
  /**
   * Resolves the specified timeline period and position to a {@link MediaSource.MediaPeriodId} that should be
   ...
   */
  public MediaSource.MediaPeriodId resolveMediaPeriodIdForAds(
   ...
   * @return The identifier for the first media period to play, taking into account unplayed ads.
   */
  private static MediaSource.MediaPeriodId resolveMediaPeriodIdForAds(
      Timeline timeline,
      Object periodUid,
      long positionUs,
    ...
    while ((period.durationUs == 0
      ...
      periodUid = Assertions.checkNotNull(period.uid);
    }
    timeline.getPeriodByUid(periodUid, period);
   ...
   * @return The identifier for the first media period to play, taking into account unplayed ads.
   */
  public MediaSource.MediaPeriodId resolveMediaPeriodIdForAdsAfterPeriodPositionChange(
      Timeline timeline, Object periodUid, long positionUs) {
    long windowSequenceNumber = resolvePeriodIndexToWindowSequenceNumber(timeline, periodUid);
    ...
    for (int i = timeline.getIndexOfPeriod(periodUid); i >= window.firstPeriodIndex; i--) {
      ...
      if (period.getAdGroupIndexForPositionUs(period.durationUs) != C.INDEX_UNSET) {
        ...
        periodUidToPlay = Assertions.checkNotNull(period.uid);
      }
  ...
  private void notifyQueueUpdate() {
    ImmutableList.Builder<MediaSource.MediaPeriodId> builder = ImmutableList.builder();
    @Nullable MediaPeriodHolder period = playing;
    while (period != null) {
      builder.add(period.info.id);
      period = period.getNext();
    }
    @Nullable MediaSource.MediaPeriodId readingPeriodId = reading == null ? null : reading.info.id;
  ...
  private MediaPeriodInfo getFollowingMediaPeriodInfo(
    ...
    if (mediaPeriodInfo.isLastInTimelinePeriod) {
      ...
      Object nextPeriodUid = Assertions.checkNotNull(period.uid);
  ...
  private MediaPeriodInfo getMediaPeriodInfo(
          Timeline timeline, MediaSource.MediaPeriodId id, long requestedContentPositionUs, long startPositionUs) {
  ...
  private MediaPeriodInfo getMediaPeriodInfoForAd(
    ...
    MediaSource.MediaPeriodId id =
        new MediaSource.MediaPeriodId(periodUid, adGroupIndex, adIndexInAdGroup, windowSequenceNumber);
    long durationUs =
        timeline
  ...
  private boolean isLastInTimeline(
          Timeline timeline, MediaSource.MediaPeriodId id, boolean isLastMediaPeriodInPeriod) {
    int periodIndex = timeline.getIndexOfPeriod(id.periodUid);
