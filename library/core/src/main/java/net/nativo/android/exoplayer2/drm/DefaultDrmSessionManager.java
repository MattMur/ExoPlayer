...
public class DefaultDrmSessionManager implements DrmSessionManager {
  ...
  public DrmSessionReference preacquireSession(
    ...
    Assertions.checkState(prepareCallsCount > 0);
    Assertions.checkStateNotNull(playbackLooper);
  ...
  private DrmSession acquireSession(
    ...
    @Nullable List<DrmInitData.SchemeData> schemeDatas = null;
    if (offlineLicenseKeySetId == null) {
      ...
      if (schemeDatas.isEmpty()) {
        ...
        return new ErrorStateDrmSession(
            new DrmSession.DrmSessionException(error, PlaybackException.ERROR_CODE_DRM_CONTENT_ERROR));
      }
    }
  ...
  public @C.CryptoType int getCryptoType(Format format) {
    @C.CryptoType int cryptoType = Assertions.checkNotNull(exoMediaDrm).getCryptoType();
    if (format.drmInitData == null) {
  ...
  private boolean canAcquireSession(DrmInitData drmInitData) {
    ...
    List<DrmInitData.SchemeData> schemeDatas = getSchemeDatas(drmInitData, uuid, true);
    if (schemeDatas.isEmpty()) {
  ...
  private static boolean acquisitionFailedIndicatingResourceShortage(DrmSession session) {
    ...
    return session.getState() == DrmSession.STATE_ERROR
        && (Util.SDK_INT < 19
            || Assertions.checkNotNull(session.getError()).getCause() instanceof ResourceBusyException);
  }
  ...
  private DefaultDrmSession createAndAcquireSession(
    ...
    DefaultDrmSession session =
        new DefaultDrmSession(
            ...
            loadErrorHandlingPolicy,
            Assertions.checkNotNull(playerId));
  ...
  /**
   * Extracts {@link DrmInitData.SchemeData} instances suitable for the given DRM scheme {@link UUID}.
   *
   ...
   *     returned.
   * @return The extracted {@link DrmInitData.SchemeData} instances, or an empty list if no suitable data is
   *     present.
   */
