...
public final class PlaybackStatsListener
  ...
  public void onSessionActive(EventTime eventTime, String sessionId) {
    Assertions.checkNotNull(playbackStatsTrackers.get(sessionId)).onForeground();
  }

  @Override
  public void onAdPlaybackStarted(
      EventTime eventTime, String contentSessionId, String adSessionId) {
    Assertions.checkNotNull(playbackStatsTrackers.get(contentSessionId)).onInterruptedByAd();
  }

  @Override
  public void onSessionFinished(
      EventTime eventTime, String sessionId, boolean automaticTransitionToNextPlayback) {
    PlaybackStatsTracker tracker = Assertions.checkNotNull(playbackStatsTrackers.remove(sessionId));
    EventTime startEventTime = Assertions.checkNotNull(sessionStartEventTimes.remove(sessionId));
  ...
  public void onEvents(Player player, Events events) {
    ...
    if (events.contains(EVENT_PLAYER_RELEASED)) {
  ...
  private static final class PlaybackStatsTracker {
    ...
    private @PlaybackStats.PlaybackState int currentPlaybackState;
    ...
    public void onEvents(
      ...
      if (fatalError != null) {
        ...
        fatalErrorCount++;
        if (keepHistory) {
          fatalErrorHistory.add(new PlaybackStats.EventTimeAndException(eventTime, fatalError));
        }
      ...
      if (nonFatalException != null) {
        nonFatalErrorCount++;
        if (keepHistory) {
          nonFatalErrorHistory.add(new PlaybackStats.EventTimeAndException(eventTime, nonFatalException));
        }
      }

      @PlaybackStats.PlaybackState int newPlaybackState = resolveNewPlaybackState(player);
    ...
    public PlaybackStats build(boolean isFinal) {
      ...
      List<PlaybackStats.EventTimeAndFormat> videoHistory =
          isFinal ? videoFormatHistory : new ArrayList<>(videoFormatHistory);
      List<PlaybackStats.EventTimeAndFormat> audioHistory =
    ...
    private void updatePlaybackState(@PlaybackStats.PlaybackState int newPlaybackState, EventTime eventTime) {
      ...
      if (keepHistory) {
        playbackStateHistory.add(new PlaybackStats.EventTimeAndPlaybackState(eventTime, currentPlaybackState));
      }
    }

    private @PlaybackStats.PlaybackState int resolveNewPlaybackState(Player player) {
    ...
    private void maybeUpdateVideoFormat(EventTime eventTime, @Nullable Format newFormat) {
      ...
      if (keepHistory) {
        videoFormatHistory.add(new PlaybackStats.EventTimeAndFormat(eventTime, currentVideoFormat));
      }
    }
    ...
    private void maybeUpdateAudioFormat(EventTime eventTime, @Nullable Format newFormat) {
      ...
      if (keepHistory) {
        audioFormatHistory.add(new PlaybackStats.EventTimeAndFormat(eventTime, currentAudioFormat));
      }
    }
    ...
    private static boolean isReadyState(@PlaybackStats.PlaybackState int state) {
    ...
    private static boolean isPausedState(@PlaybackStats.PlaybackState int state) {
      return state == PlaybackStats.PLAYBACK_STATE_PAUSED
          || state == PlaybackStats.PLAYBACK_STATE_PAUSED_BUFFERING;
    }

    private static boolean isRebufferingState(@PlaybackStats.PlaybackState int state) {
    ...
    private static boolean isInvalidJoinTransition(
            @PlaybackStats.PlaybackState int oldState, @PlaybackStats.PlaybackState int newState) {
