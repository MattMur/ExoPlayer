...
import net.nativo.android.exoplayer2.analytics.AnalyticsListener.EventTime;
...
public class DefaultAnalyticsCollector implements AnalyticsCollector {
  ...
  public void setPlayer(Player player, Looper looper) {
    Assertions.checkState(this.player == null || mediaPeriodQueueTracker.mediaPeriodQueue.isEmpty());
  ...
  public void release() {
    ...
    Assertions.checkStateNotNull(handler).post(this::releaseInternal);
  }

  @Override
  public final void updateMediaPeriodQueueInfo(
      List<MediaPeriodId> queue, @Nullable MediaPeriodId readingPeriod) {
    mediaPeriodQueueTracker.onQueueUpdated(queue, readingPeriod, Assertions.checkNotNull(player));
  }
  ...
  public final void onTimelineChanged(Timeline timeline, @Player.TimelineChangeReason int reason) {
    mediaPeriodQueueTracker.onTimelineChanged(Assertions.checkNotNull(player));
    EventTime eventTime = generateCurrentPlayerMediaPeriodEventTime();
    sendEvent(
        eventTime,
  ...
  private static final class MediaPeriodQueueTracker {
    ...
    public void onQueueUpdated(
      ...
      if (!queue.isEmpty()) {
        ...
        readingMediaPeriod = Assertions.checkNotNull(readingPeriod);
      }
