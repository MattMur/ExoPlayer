    ...
    for (int i = 0; i < periods.length; i++) {
      for (int j = 0; j < selections.length; j++) {
        ...
        if (selectionChildIndices[j] == i) {
          ExoTrackSelection mergedTrackSelection = Assertions.checkNotNull(selections[j]);
          TrackGroup mergedTrackGroup = mergedTrackSelection.getTrackGroup();
          TrackGroup childTrackGroup =
              Assertions.checkNotNull(childTrackGroupByMergedTrackGroup.get(mergedTrackGroup));
