...
import net.nativo.android.exoplayer2.drm.DrmSessionEventListener.EventDispatcher;
import net.nativo.android.exoplayer2.drm.DrmSessionManager;
import net.nativo.android.exoplayer2.drm.DrmSessionManager.DrmSessionReference;
...
public class SampleQueue implements TrackOutput {
  ...
  public int read(
    ...
    int result =
        peekSampleMetadata(
            formatHolder,
            buffer,
            /* formatRequired= */ (readFlags & SampleStream.FLAG_REQUIRE_FORMAT) != 0,
            loadingFinished,
            extrasHolder);
    if (result == C.RESULT_BUFFER_READ && !buffer.isEndOfStream()) {
      boolean peek = (readFlags & SampleStream.FLAG_PEEK) != 0;
  ...
  public final synchronized void skip(int count) {
    Assertions.checkArgument(count >= 0 && readPosition + count <= length);
    readPosition += count;
  }
  ...
  private synchronized void commitSample(
    ...
    if (sharedSampleMetadata.isEmpty()
      ...
      sharedSampleMetadata.appendSpan(
          getWriteIndex(),
          new SharedSampleMetadata(Assertions.checkNotNull(upstreamFormat), drmSessionReference));
    }
