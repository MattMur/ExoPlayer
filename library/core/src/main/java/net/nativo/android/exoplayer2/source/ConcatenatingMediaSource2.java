...
public final class ConcatenatingMediaSource2 extends CompositeMediaSource<Integer> {
  ...
  public static final class Builder {
    ...
    public Builder setMediaSourceFactory(MediaSource.Factory mediaSourceFactory) {
      this.mediaSourceFactory = Assertions.checkNotNull(mediaSourceFactory);
      return this;
    }
    ...
    public ConcatenatingMediaSource2 build() {
      Assertions.checkArgument(index > 0, "Must add at least one source to the concatenation.");
      if (mediaItem == null) {
  ...
  private void scheduleTimelineUpdate() {
    if (!timelineUpdateScheduled) {
      Assertions.checkNotNull(playbackThreadHandler).obtainMessage(MSG_UPDATE_TIMELINE).sendToTarget();
  ...
  private static final class ConcatenatedTimeline extends Timeline {
    ...
    public final Period getPeriod(int periodIndex, Period period, boolean setIds) {
      ...
      period.positionInWindowUs = periodOffsetsInWindowUs.get(periodIndex);
      if (setIds) {
        period.uid = getPeriodUid(childIndex, Assertions.checkNotNull(period.uid));
      }
      return period;
    }
