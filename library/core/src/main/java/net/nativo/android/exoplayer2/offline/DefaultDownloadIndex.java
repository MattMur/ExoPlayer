...
public final class DefaultDownloadIndex implements WritableDownloadIndex {
  ...
  private static Download getDownloadForCurrentRow(Cursor cursor) {
    ...
    DownloadRequest request =
        new DownloadRequest.Builder(
                /* id= */ Assertions.checkNotNull(cursor.getString(COLUMN_INDEX_ID)),
                /* uri= */ Uri.parse(Assertions.checkNotNull(cursor.getString(COLUMN_INDEX_URI))))
    ...
    @Download.State int state = cursor.getInt(COLUMN_INDEX_STATE);
  ...
  private static Download getDownloadForCurrentRowV2(Cursor cursor) {
    ...
    DownloadRequest request =
        new DownloadRequest.Builder(
                /* id= */ Assertions.checkNotNull(cursor.getString(0)),
                /* uri= */ Uri.parse(Assertions.checkNotNull(cursor.getString(2))))
    ...
    @Download.State int state = cursor.getInt(6);
    // It's possible the database contains failure reasons for non-failed downloads, which is
    // invalid. Clear them here. See https://github.com/google/ExoPlayer/issues/6785.
    @Download.FailureReason
    int failureReason =
