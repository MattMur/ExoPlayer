...
import net.nativo.android.exoplayer2.util.EGLSurfaceTexture;
...
public final class PlaceholderSurface extends Surface {
  ...
  private static @EGLSurfaceTexture.SecureMode int secureMode;
  ...
  public static synchronized boolean isSecureSupported(Context context) {
    ...
    return secureMode != EGLSurfaceTexture.SECURE_MODE_NONE;
  }
  ...
  public static PlaceholderSurface newInstanceV17(Context context, boolean secure) {
    ...
    return thread.init(secure ? secureMode : EGLSurfaceTexture.SECURE_MODE_NONE);
  }
  ...
  private static @EGLSurfaceTexture.SecureMode int getSecureMode(Context context) {
    if (GlUtil.isProtectedContentExtensionSupported(context)) {
      if (GlUtil.isSurfacelessContextExtensionSupported()) {
        return EGLSurfaceTexture.SECURE_MODE_SURFACELESS_CONTEXT;
      } else {
        ...
        return EGLSurfaceTexture.SECURE_MODE_PROTECTED_PBUFFER;
      }
    } else {
      return EGLSurfaceTexture.SECURE_MODE_NONE;
    }
  }
  ...
  private static class PlaceholderSurfaceThread extends HandlerThread implements Handler.Callback {
    ...
    public PlaceholderSurface init(@EGLSurfaceTexture.SecureMode int secureMode) {
      start();
    ...
    private void initInternal(@EGLSurfaceTexture.SecureMode int secureMode) throws GlUtil.GlException {
      Assertions.checkNotNull(eglSurfaceTexture);
      eglSurfaceTexture.init(secureMode);
      this.surface =
          new PlaceholderSurface(
              this, eglSurfaceTexture.getSurfaceTexture(), secureMode != EGLSurfaceTexture.SECURE_MODE_NONE);
    }
