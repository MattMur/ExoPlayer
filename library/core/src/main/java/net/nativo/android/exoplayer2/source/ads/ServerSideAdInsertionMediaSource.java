...
import static net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionUtil.getAdCountInGroup;
import static net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionUtil.getMediaPeriodPositionUs;
import static net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionUtil.getMediaPeriodPositionUsForAd;
import static net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionUtil.getMediaPeriodPositionUsForContent;
import static net.nativo.android.exoplayer2.source.ads.ServerSideAdInsertionUtil.getStreamPositionUs;
  ...
  public MediaPeriod createPeriod(MediaPeriodId id, Allocator allocator, long startPositionUs) {
    ...
    if (sharedPeriod == null) {
      ...
      if (lastExistingPeriod != null
        ...
        AdPlaybackState adPlaybackState = Assertions.checkNotNull(adPlaybackStates.get(id.periodUid));
    ...
    if (mediaPeriod == null) {
      ...
      mediaPeriod.mediaSourceEventDispatcher.loadStarted(
          ...
          correctMediaLoadData(
              mediaPeriod,
              mediaLoadData,
              Assertions.checkNotNull(adPlaybackStates.get(mediaPeriod.mediaPeriodId.periodUid))));
    }
  }
  ...
  private static final class SharedMediaPeriod implements MediaPeriod.Callback {
    ...
    public void prepare(MediaPeriodImpl mediaPeriod, long positionUs) {
      ...
      if (hasStartedPreparing) {
        if (isPrepared) {
          Assertions.checkNotNull(mediaPeriod.callback).onPrepared(mediaPeriod);
        }
        return;
      }
    ...
    public @SampleStream.ReadDataResult int readData(
      ...
      int result =
          Util.castNonNull(sampleStreams[streamIndex]).readData(formatHolder, buffer, peekingFlags);
      long adjustedTimeUs =
      ...
      if (result == C.RESULT_BUFFER_READ) {
        maybeNotifyDownstreamFormatChanged(mediaPeriod, streamIndex);
        Util.castNonNull(sampleStreams[streamIndex]).readData(formatHolder, buffer, readFlags);
    ...
    public int skipData(MediaPeriodImpl mediaPeriod, int streamIndex, long positionUs) {
      long actualPositionUs =
          getStreamPositionUs(positionUs, mediaPeriod.mediaPeriodId, adPlaybackState);
      return Util.castNonNull(sampleStreams[streamIndex]).skipData(actualPositionUs);
    }

    public boolean isReady(int streamIndex) {
      return Util.castNonNull(sampleStreams[streamIndex]).isReady();
    }

    public void maybeThrowError(int streamIndex) throws IOException {
      Util.castNonNull(sampleStreams[streamIndex]).maybeThrowError();
    }
    ...
    public void onContinueLoadingRequested(MediaPeriod source) {
      ...
      Assertions.checkNotNull(loadingPeriod.callback).onContinueLoadingRequested(loadingPeriod);
    }
  ...
  private static final class ServerSideAdInsertionTimeline extends ForwardingTimeline {
    ...
    public Window getWindow(int windowIndex, Window window, long defaultPositionProjectionUs) {
      ...
      Object firstPeriodUid =
          Assertions.checkNotNull(getPeriod(window.firstPeriodIndex, period, /* setIds= */ true).uid);
      AdPlaybackState firstAdPlaybackState = Assertions.checkNotNull(adPlaybackStates.get(firstPeriodUid));
      ...
      if (window.durationUs == C.TIME_UNSET) {
        ...
        AdPlaybackState lastAdPlaybackState =
            Assertions.checkNotNull(adPlaybackStates.get(originalLastPeriod.uid));
    ...
    public Period getPeriod(int periodIndex, Period period, boolean setIds) {
      ...
      AdPlaybackState adPlaybackState = Assertions.checkNotNull(adPlaybackStates.get(period.uid));
      ...
      for (int i = 0; i < periodIndex + 1; i++) {
        ...
        AdPlaybackState innerAdPlaybackState = Assertions.checkNotNull(adPlaybackStates.get(innerPeriod.uid));
        if (i == 0) {
          positionInWindowUs =
              -getMediaPeriodPositionUsForContent(
