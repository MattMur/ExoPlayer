...
public abstract class MappingTrackSelector extends TrackSelector {
  ...
  public static final class MappedTrackInfo {
     ...
     * @param groupIndex The index of the track group to which the track belongs.
     * @param trackIndex The index of the track within the track group.
     * @return The {@link C.FormatSupport}.
     */
    public @C.FormatSupport int getTrackSupport(int rendererIndex, int groupIndex, int trackIndex) {
    ...
    public @AdaptiveSupport int getAdaptiveSupport(
      ...
      for (int i = 0; i < trackCount; i++) {
        @C.FormatSupport int fixedSupport = getTrackSupport(rendererIndex, groupIndex, i);
  ...
  private static int findRenderer(
    ...
    @C.FormatSupport int bestFormatSupportLevel = C.FORMAT_UNSUPPORTED_TYPE;
    boolean bestRendererIsUnassociated = true;
    for (int rendererIndex = 0; rendererIndex < rendererCapabilities.length; rendererIndex++) {
      RendererCapabilities rendererCapability = rendererCapabilities[rendererIndex];
      @C.FormatSupport int formatSupportLevel = C.FORMAT_UNSUPPORTED_TYPE;
