 ...
 */
package net.nativo.android.exoplayer2.source.mediaparser;
...
import net.nativo.android.exoplayer2.extractor.DummyExtractorOutput;
...
public final class OutputConsumerAdapterV30 implements MediaParser.OutputConsumer {
  ...
  @Nullable
  private TrackOutput.CryptoData toExoPlayerCryptoData(int trackIndex, @Nullable CryptoInfo cryptoInfo) {
    if (cryptoInfo == null) {
      return null;
    }
    ...
    if (lastReceivedCryptoInfo == cryptoInfo) {
      ...
      cryptoDataToOutput =
          new TrackOutput.CryptoData(cryptoInfo.mode, cryptoInfo.key, encryptedBlocks, clearBlocks);
  ...
  private static DrmInitData toExoPlayerDrmInitData(
    ...
    for (int i = 0; i < schemeDatas.length; i++) {
      ...
      schemeDatas[i] =
          new DrmInitData.SchemeData(schemeInitData.uuid, schemeInitData.mimeType, schemeInitData.data);
    }
    return new DrmInitData(schemeType, schemeDatas);
  }

  private static @C.SelectionFlags int getSelectionFlags(MediaFormat mediaFormat) {
