...
import net.nativo.android.exoplayer2.drm.UnsupportedDrmException;
import net.nativo.android.exoplayer2.mediacodec.MediaCodecDecoderException;
...
import net.nativo.android.exoplayer2.util.NetworkTypeObserver;
...
public final class MediaMetricsListener
    ...
    if (eventTime.mediaPeriodId != null) {
      String sessionId =
          sessionManager.getSessionForMediaPeriodId(
              eventTime.timeline, Assertions.checkNotNull(eventTime.mediaPeriodId));
  ...
  public void onDownstreamFormatChanged(EventTime eventTime, MediaLoadData mediaLoadData) {
    ...
    PendingFormatUpdate update =
        new PendingFormatUpdate(
            Assertions.checkNotNull(mediaLoadData.trackFormat),
  ...
  private void maybeUpdateMetricsBuilderValues(Player player, Events events) {
    ...
    if (events.contains(EVENT_TRACKS_CHANGED) && metricsBuilder != null) {
      ...
      if (drmInitData != null) {
        Util.castNonNull(metricsBuilder).setDrmType(getDrmType(drmInitData));
      }
    }
  ...
  private static ErrorInfo getErrorInfo(
    ...
    Throwable cause = Assertions.checkNotNull(error.getCause());
    if (cause instanceof IOException) {
      if (cause instanceof HttpDataSource.InvalidResponseCodeException) {
        ...
        cause = Assertions.checkNotNull(cause.getCause());
        ...
        @Nullable Throwable notFoundCause = Assertions.checkNotNull(cause.getCause()).getCause();
        if (Util.SDK_INT >= 21
