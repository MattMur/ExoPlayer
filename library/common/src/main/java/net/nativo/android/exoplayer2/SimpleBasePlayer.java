...
import net.nativo.android.exoplayer2.util.Size;
...
public abstract class SimpleBasePlayer extends BasePlayer {
  ...
  protected static final class State {
    ...
    public static final class Builder {
      ...
      public Builder() {
        ...
        positionDiscontinuityReason = DISCONTINUITY_REASON_INTERNAL;
      ...
      public Builder setCurrentAd(int adGroupIndex, int adIndexInAdGroup) {
        Assertions.checkArgument((adGroupIndex == C.INDEX_UNSET) == (adIndexInAdGroup == C.INDEX_UNSET));
    ...
    private State(Builder builder) {
      if (builder.timeline.isEmpty()) {
        Assertions.checkArgument(
            builder.playbackState == STATE_IDLE
                || builder.playbackState == STATE_ENDED,
      ...
      if (builder.playerError != null) {
        Assertions.checkArgument(
            builder.playbackState == STATE_IDLE, "Player error only allowed in STATE_IDLE");
      }
      if (builder.playbackState == STATE_IDLE
          || builder.playbackState == STATE_ENDED) {
        Assertions.checkArgument(
      ...
      if (builder.contentPositionMs != null) {
        if (builder.currentAdGroupIndex == C.INDEX_UNSET
            ...
            && builder.playbackState == STATE_READY
            && builder.playbackSuppressionReason == PLAYBACK_SUPPRESSION_REASON_NONE
  ...
  private static final class PlaylistTimeline extends Timeline {
    ...
    public Period getPeriodByUid(Object periodUid, Period period) {
      int periodIndex = Assertions.checkNotNull(periodIndexByUid.get(periodUid));
  ...
  public final void setPlayWhenReady(boolean playWhenReady) {
    ...
    if (!shouldHandleCommand(COMMAND_PLAY_PAUSE)) {
      return;
    }
  ...
  private void setMediaItemsInternal(
      List<MediaItem> mediaItems, int startIndex, long startPositionMs) {
    Assertions.checkArgument(startIndex == C.INDEX_UNSET || startIndex >= 0);
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_CHANGE_MEDIA_ITEMS)
        && (mediaItems.size() != 1 || !shouldHandleCommand(COMMAND_SET_MEDIA_ITEM))) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void addMediaItems(int index, List<MediaItem> mediaItems) {
    ...
    if (!shouldHandleCommand(COMMAND_CHANGE_MEDIA_ITEMS) || mediaItems.isEmpty()) {
      return;
    }
  ...
  public final void prepare() {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_PREPARE)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setRepeatMode(@Player.RepeatMode int repeatMode) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_REPEAT_MODE)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setShuffleModeEnabled(boolean shuffleModeEnabled) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_SHUFFLE_MODE)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setPlaybackParameters(PlaybackParameters playbackParameters) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_SPEED_AND_PITCH)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void stop() {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_STOP)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setTrackSelectionParameters(TrackSelectionParameters parameters) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_TRACK_SELECTION_PARAMETERS)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setPlaylistMetadata(MediaMetadata mediaMetadata) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_MEDIA_ITEMS_METADATA)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setVolume(float volume) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_VOLUME)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void setVideoSurface(@Nullable Surface surface) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_VIDEO_SURFACE)) {
      return;
    }
    if (surface == null) {
      clearVideoSurface();
  ...
  public final void setDeviceVolume(int volume) {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_SET_DEVICE_VOLUME)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  public final void increaseDeviceVolume() {
    ...
    // Use a local copy to ensure the lambda below uses the current state value.
    State state = this.state;
    if (!shouldHandleCommand(COMMAND_ADJUST_DEVICE_VOLUME)) {
      return;
    }
    updateStateForPendingOperation(
  ...
  private void updateStateAndInformListeners(
    ...
    if (!Util.areEqual(previousState.playerError, newState.playerError)) {
      ...
      if (newState.playerError != null) {
        listeners.queueEvent(
                EVENT_PLAYER_ERROR,
            listener -> listener.onPlayerError(Util.castNonNull(newState.playerError)));
      }
    }
    ...
    if (!previousTracks.equals(newTracks)) {
      listeners.queueEvent(
              EVENT_TRACKS_CHANGED, listener -> listener.onTracksChanged(newTracks));
    }
    ...
    if (newState.newlyRenderedFirstFrame) {
      listeners.queueEvent(EVENT_RENDERED_FIRST_FRAME, Listener::onRenderedFirstFrame);
    }
    ...
    if (previousState.volume != newState.volume) {
      listeners.queueEvent(
              EVENT_VOLUME_CHANGED, listener -> listener.onVolumeChanged(newState.volume));
    }
    ...
    if (!previousState.timedMetadata.equals(newState.timedMetadata)
      ...
      listeners.queueEvent(
              EVENT_METADATA, listener -> listener.onMetadata(newState.timedMetadata));
    }
  ...
  private void updateStateForPendingOperation(
    ...
    if (pendingOperation.isDone() && pendingOperations.isEmpty()) {
      ...
      pendingOperation.addListener(
          () -> {
            Util.castNonNull(state); // Already checked by method @RequiresNonNull pre-condition.
  ...
  private static boolean isPlaying(State state) {
    return state.playWhenReady
        && state.playbackState == STATE_READY
  ...
  private static int getPeriodIndexFromWindowPosition(
    ...
    Object periodUid =
        timeline.getPeriodPositionUs(window, period, windowIndex, Util.msToUs(windowPositionMs)).first;
  ...
  private static State getStateWithNewPlaylist(
    ...
    if (oldState.playbackState != STATE_IDLE && newIndex == C.INDEX_UNSET) {
      stateBuilder.setPlaybackState(STATE_ENDED).setIsLoading(false);
    }
  ...
  private static State getStateWithNewPlaylistAndPosition(
    ...
    State.Builder stateBuilder = oldState.buildUpon();
    stateBuilder.setPlaylist(newPlaylist);
    if (oldState.playbackState != STATE_IDLE) {
      if (newPlaylist.isEmpty()) {
        stateBuilder.setPlaybackState(STATE_ENDED).setIsLoading(false);
      } else {
        stateBuilder.setPlaybackState(STATE_BUFFERING);
      }
    }
    long oldPositionMs = oldState.contentPositionMsSupplier.get();
    return buildStateForNewPosition(
  ...
  private static State buildStateForNewPosition(
    ...
    if (!newPlaylist.isEmpty() && newPositionMs == C.TIME_UNSET) {
      newPositionMs = Util.usToMs(newPlaylist.get(newIndex).defaultPositionUs);
    }
